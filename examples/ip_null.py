...
'''
    ...
    Trivial example of how to null (remove from ban/whitelists) an IP. See:
...
if __name__ == '__main__':
    ip = '8.8.8.8'
    print '> nulling IP: {0}'.format(ip)
    cloudflare = txcloudflare.client_api(email_address, api_token)
    cloudflare.nul(ip=ip).addCallback(got_response).addErrback(got_error)
    reactor.run()
